    ...
    <div class="results-container">
      ...
      <div class="score-section">
        ...
        <div class="processing-info" v-if="!allProcessed">
          <p>Processing {{ formatCount(processedCount) }} of {{ totalQuestionCount }} questions...</p>
        </div>
      </div>
      ...
      <div class="results-details">
        ...
        <div v-else-if="resultsData" class="question-results">
          ...
          <div class="results-summary">
            <p><strong>Final Score:</strong> {{ formatScore(score) }}/{{ totalQuestions }} ({{ percentage }}%)</p>
            <p><strong>Time Taken:</strong> {{ formatDuration(resultsData.duration_seconds) }}</p>
          </div>

          <!-- AI Disclaimer -->
          <div class="ai-disclaimer">
            <p>⚠️ <strong>AI-Generated Content:</strong> All feedback, explanations, and suggested answers are generated by artificial intelligence. While we strive for accuracy, AI systems may occasionally produce incorrect or incomplete information (AI hallucinations). Please use this feedback as a learning tool rather than definitive assessment.</p>
          </div>
            ...
            <div v-for="question in resultsData.question_results" :key="question.question_index" class="question-item">
              ...
              <div v-if="['read_aloud', 'quick_response', 'translation'].includes(question.question_type) && question.student_audio_path" class="audio-playback">
                <button @click="playStudentAudio(question.student_audio_path, question.question_index)" class="btn btn-audio" :disabled="isPlayingAudio === question.question_index">
                  {{ isPlayingAudio === question.question_index ? '🔊 Playing...' : '🔊 Play Student Answer' }}
                </button>
              </div>
              ...
              <div v-if="question.question_type === 'multiple_choice'" class="answer-display">
                <p><strong>Your Answer:</strong> {{ question.student_answer || 'Not answered' }}</p>
                <p><strong>Correct Answer:</strong> {{ question.reference_answer }}</p>
              </div>
              ...
              <div class="question-feedback">
                ...
                <p v-if="question.suggested_answer"><strong>Suggested Answer:</strong> {{ question.suggested_answer }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-else class="placeholder-message">
          ...
          <p>Finalizing your exam results...</p>
        </div>
      </div>
